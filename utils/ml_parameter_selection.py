...
class ML_Param_Selection():
    ...
    def xgb_objective(self,trial):
        ...
        scale_pos_weight  = trial.suggest_float('scale_pos_weight',1,2) if self.is_unbalance else 1
        ...
        score = sklearn.model_selection.cross_val_score(model, self.X, self.y, n_jobs=-1, cv=self.cv,scoring=scorer)
    ...
    def LR_objective(self,trial):
        ...
        positive_class_weight  = trial.suggest_float('class_weight',0.3,0.7) if self.is_unbalance else 0.5
    ...
    def random_forest_obj(self,trial):
        ...
        # 选择五则交叉下的ROC均值为参数评价
        scorer = make_scorer(self.scorer, pos_label=1)
        score = sklearn.model_selection.cross_val_score(classifier_obj, self.X, self.y, n_jobs=-1, cv=cv,scoring=scorer)
        return score.mean()
